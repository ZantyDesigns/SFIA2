                 ...
                 script{
                     if (env.rollback == 'false'){
                          withCredentials([string(credentialsId: 'DATABASE_URI', variable: 'DBURI'), string(credentialsId: 'MYSQL_ROOT_PASSWORD', variable: 'SQLPASS'), string(credentialsId: 'SECRET_KEY', variable: 'SECRET'), file(credentialsId: 'TEST_PEM', variable: 'TEST_PEM')]) {
                               sh '''
                                  # SSH into testing-vm
                                  ssh -tt -o "StrictHostKeyChecking=no" -i $TEST_PEM ubuntu@ec2-18-134-3-111.eu-west-2.compute.amazonaws.com << EOF
                             ...
                             if (env.rollback == 'false'){
                                  withCredentials([string(credentialsId: 'DATABASE_URI', variable: 'DBURI'), string(credentialsId: 'MYSQL_ROOT_PASSWORD', variable: 'SQLPASS'), string(credentialsId: 'SECRET_KEY', variable: 'SECRET'), file(credentialsId: 'TEST_PEM', variable: 'TEST_PEM')]) {
                                       sh '''
                                          ...
                                          docker-compose exec -T frontend pytest --cov application >> frontend_test.txt
                                          docker-compose exec -T backend pytest --cov application >> backend_test.txt
                                          exit
                                          >> EOF
                                          '''
                                  }
