                     ...
                     if (env.rollback == 'false'){
                          withCredentials([string(credentialsId: 'DATABASE_URI', variable: 'DBURI'), string(credentialsId: 'TEST_DATABASE_URI', variable: 'TESTDBURI'), string(credentialsId: 'MYSQL_ROOT_PASSWORD', variable: 'SQLPASS'), string(credentialsId: 'SECRET_KEY', variable: 'SECRET'), file(credentialsId: 'TEST_PEM', variable: 'TEST_PEM')]) {
                               sh '''
                                  ...
                                  sudo -E MYSQL_ROOT_PASSWORD=$SQLPASS DATABASE_URI=$DBURI TEST_DATABASE_URI=$TESTDBURI SECRET_KEY=$SECRET docker exec frontend pytest --cov-report term --cov application
                                  sudo -E MYSQL_ROOT_PASSWORD=$SQLPASS DATABASE_URI=$DBURI TEST_DATABASE_URI=$TESTDBURI SECRET_KEY=$SECRET docker exec backend pytest --cov-report term --cov application
                                  exit
                                  >> EOF
                                  '''
                          }
